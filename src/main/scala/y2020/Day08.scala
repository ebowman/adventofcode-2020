...
trait Day08 extends RegexParsers {
  ...
  def nopsAndJumps(instructions: Seq[Instruction]): Seq[Int] = instructions.zipWithIndex.filter(x => x._1.isInstanceOf[Nop] || x._1.isInstanceOf[Jmp]).map(_._2)

  case class ChangeMachine(instructions: IndexedSeq[Instruction], nopsAndJumps: Seq[Int], result: Int = -1) {
    ...
    def flip(instructions: IndexedSeq[Instruction], c: Int): IndexedSeq[Instruction] = {
      instructions(c) match {
        case Nop(x) =>
          (instructions.take(c) :+ Jmp(x)) ++ instructions.drop(c + 1)
        case Jmp(x) =>
          (instructions.take(c) :+ Nop(x)) ++ instructions.drop(c + 1)
        case _ => sys.error(s"")
      }
    }
  ...
  case class MetaMachine(instructions: IndexedSeq[Instruction]) {
    def run: Int = Machine(instructions).run match {
      ...
      case (false, _) =>
        ChangeMachine(instructions, nopsAndJumps(instructions)).solve
    }
  }
    ...
    def run: (Boolean, Int) = {
      ...
      def recurse(cursor: Int = 0, accum: Int = 0, seen: Set[Int] = Set()): (Boolean, Int) = {
        if (cursor == instructions.length) (true, accum)
          ...
          if (seen.contains(cursor)) (false, accum)
          else {
  ...
  class Compiler extends RegexParsers {
    ...
    def compile: Parser[Instruction] = nop | jmp | acc

    def compile(program: Iterable[String]): IndexedSeq[Instruction] =
      program.map { line => parseAll(compile, line).get }.toIndexedSeq
  }
