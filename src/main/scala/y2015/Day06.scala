...
object Lights {
  ...
  case class TurnOn(range: Rect) extends Command {
    override def operate(grid: Grid): Grid = { grid.turnOn(range); grid }
  }

  case class TurnOff(range: Rect) extends Command {
    override def operate(grid: Grid): Grid = { grid.turnOff(range); grid }
  }

  case class Toggle(range: Rect) extends Command {
    override def operate(grid: Grid): Grid = { grid.toggle(range); grid }
  }
